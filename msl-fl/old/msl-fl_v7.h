...
public:
	...
	// Add
	void Add(VString key, VString val){
		msl_value *p=New();
		p->val=val; p->key=key;
		return ;
	}
...
class msl_fl{
	...
	void DoCode(unsigned char*&line, unsigned char *to, msl_value &outval, unsigned char ecode=1, unsigned char ecodet=1){
		msl_value *value=0; msl_value valueline;
		...
		while(line<to && !do_opt_stopit){
			...
			// operators
			else if(*line=='+' || *line=='-' || *line=='*' || *line=='/' || *line=='.' || *line=='!' || *line=='<' || *line=='>' || *line=='='){
				pline=line++;
				while(*line=='+' || *line=='-' || *line=='*' || *line=='/' || *line=='.' || *line=='!' || *line=='<' || *line=='>' || *line=='=') line++;
				if(line>=to){ SetError("EOF"); return ; }
				
				VString name(pline, line-pline);
				// =
				if(*(line-1)=='=' && !((*(pline)=='=' || *(pline)=='>' || *(pline)=='<' ) && line-pline==2)){
					msl_value val;
			...
			for(val; val && sz!=1; val=val->_n){
				...
				case 0:
					if(val->key=="o" && val->val=="!"){
						...
						val->key="t"; val->val= (!val->_n->val || val->_n->val!="0");
						line.Del(val->_n); sz--;
					}
					break;
				case 1:
					if(val->key=="o"){
						...
						if(i==1 && (val->val=="*" || val->val=="/" || val->val=="%")){
							if(val->val=="/" && !val->_n->val.toi()){
								...
								if(val->val=="*") itos(val->val= val->_p->val.toi()*val->_n->val.toi());
						...
						if(i==5 && (val->val=="==" || val->val=="!=")){
							val->val=val->val=="==" ? val->_p->val==val->_n->val : val->_p->val!=val->_n->val;
						}else
						// & ^ |
						...
						if(i==7 && (val->val=="&&" || val->val=="||")){
							...
							int r=val->_p->val && val->_p->val!="0";
							val->val=val->val=="&&" ? l && r : l || r;
						}
	...
	void DoCodeFunction(unsigned char*&line, unsigned char *to, msl_value &val){
